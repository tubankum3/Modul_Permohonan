...
with open('components/eadvo_EditPutusan.tsx', 'r') as f:
    content = f.read()

# Let's replace the whole CrudModal with a cleaner version
...
if old_crud_modal:
    ...
    const renderField = (field: { name: string, label: string, type: string, options?: string[] }) => {
        return (
            <div key={field.name}>
                ...
                {field.type === 'textarea' ? (
                    <textarea name={field.name} value={formData[field.name] || ''} onChange={handleChange} rows={3} className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                ) : field.type === 'richtext' ? (
    ...
    with open('components/eadvo_EditPutusan.tsx', 'w') as f:
        f.write(content)
    print("Replaced CrudModal in eadvo_EditPutusan.tsx")
else:
    print("Could not find CrudModal in eadvo_EditPutusan.tsx")
